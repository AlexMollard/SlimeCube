...
void MaterialComponentPanel::OnRender(std::shared_ptr<ResourceHub> resHub, MaterialComponent& component, ImGuiWindowFlags flags)
{
	...
	if (auto tex = DragAndDropTexture(resHub, component))
		component.material->SetAlbedo(tex);
	ImGui::NextColumn();
	...
	ImGui::ImageButton(id, imageSize, { 0, 1 }, { 1, 0 });
	if (auto tex = DragAndDropTexture(resHub, component))
		component.material->SetSpecMap(tex);
	ImGui::NextColumn();
	...
	ImGui::ImageButton(id, imageSize, { 0, 1 }, { 1, 0 });
	if (auto tex = DragAndDropTexture(resHub, component))
		component.material->SetNormalMap(tex);
	ImGui::NextColumn();
	...
	ImGui::ImageButton(id, imageSize, { 0, 1 }, { 1, 0 });
	if (auto tex = DragAndDropTexture(resHub, component))
		component.material->SetAmbientMap(tex);
	ImGui::NextColumn();
	...
	ImGui::ImageButton(id, imageSize, { 0, 1 }, { 1, 0 });
	if (auto tex = DragAndDropTexture(resHub, component))
		component.material->SetRoughMap(tex);
	ImGui::NextColumn();
	...
	ImGui::ImageButton(id, imageSize, { 0, 1 }, { 1, 0 });
	if (auto tex = DragAndDropTexture(resHub, component))
		component.material->SetDisplacementMap(tex);
...
std::shared_ptr<Texture> MaterialComponentPanel::DragAndDropTexture(std::shared_ptr<ResourceHub> resHub, MaterialComponent& component)
{
