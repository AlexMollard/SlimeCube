...
void ImGuiLayer::SetScene(Scene *scene) { GetInstance()->scene = scene; }

void ImGuiLayer::DeleteInstance() { delete GetInstance(); }
...
void ImGuiLayer::SetGizmoState(ImGuizmo::OPERATION newState) { GetInstance()->gizmoType = newState; }

ImVec2 ImGuiLayer::GetViewPortSize() { return GetInstance()->viewPortSize; }
...
void ImGuiLayer::Hierarchy()
{
 ...
 if (ImGui::BeginPopupContextWindow(nullptr, 1, false)) {
  if (ImGui::MenuItem("Create Empty Object")) selectionContext = scene->CreateEntity("Empty Object");
...
void ImGuiLayer::DrawEntityNode(Entity entity)
{
 ...
 if (opened) {
  ...
  opened = ImGui::TreeNodeEx((void *)9817239, treeNodeFlags, tag.c_str());
...
void ImGuiLayer::Properties(Entity entity) const
{
 ...
 if (entity) DrawProperties(entity);
...
static void DrawVec3Control(const std::string &label, glm::vec3 &values)
{
 ...
 if (ImGui::Button("X", buttonSize)) values.x = 0.0;
 ImGui::PopFont();
 ...
 ImGui::PushFont(boldFont);
 if (ImGui::Button("Y", buttonSize)) values.y = 0.0;
 ImGui::PopFont();
 ImGui::PopStyleColor(3);
 ...
 ImGui::PushFont(boldFont);
 if (ImGui::Button("Z", buttonSize)) values.z = 0.0;
 ImGui::PopFont();
 ImGui::PopStyleColor(3);
...
void ImGuiLayer::DrawProperties(Entity entity)
{
 ...
 if (entity.HasComponent<SkyBoxComponent>()) {
  DrawComponent<SkyBoxComponent>("SkyBox", entity, [](auto &component) {
   DrawVec3Control("Position", component.m_camera->Position);
   component.m_camera->UpdateProjectionViewMatrix();
  });
 }
}
...
{
 contentBrowser.OnRender(GetInstance()->scene->GetResourceHub(), ImVec2(0.0f, screenSize.y - rowHeight),
     ImVec2((screenSize.x - (columnWidth)), rowHeight + 1.0f), mainFlags);
}
