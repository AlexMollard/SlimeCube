...
void ImGuiLayer::DrawProperties(Entity entity)
{
    ...
    if (entity.HasComponent<PointLightComponent>()) {
        DrawComponent<PointLightComponent>("PointLight", entity, [](auto& component) {
            GetInstance()->pointLightComponentPanel.OnRender(
                GetInstance()->scene->GetResourceHub(), component, GetInstance()->mainFlags);
        });
    }

    if (entity.HasComponent<MaterialComponent>()) {
        DrawComponent<MaterialComponent>("Material", entity, [](auto& component) {
            GetInstance()->materialComponentPanel.OnRender(
                GetInstance()->scene->GetResourceHub(), component, GetInstance()->mainFlags);
        });
    }
