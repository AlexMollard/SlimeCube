...
void SlimeCube::ProcessMovement(float deltaTime)
{
 ...
 if (inputManager->GetMouseDown(1)) {
  ...
  if (inputManager->GetKeyPress(Keycode::W)) camera->ProcessKeyboard(Camera_FORWARD, deltaTime);
  if (inputManager->GetKeyPress(Keycode::S)) camera->ProcessKeyboard(Camera_BACKWARD, deltaTime);
  if (inputManager->GetKeyPress(Keycode::A)) camera->ProcessKeyboard(Camera_LEFT, deltaTime);
  if (inputManager->GetKeyPress(Keycode::D)) camera->ProcessKeyboard(Camera_RIGHT, deltaTime);
  if (inputManager->GetKeyPress(Keycode::SPACE)) camera->ProcessKeyboard(Camera_UP, deltaTime);
  if (inputManager->GetKeyPress(Keycode::LEFT_CONTROL)) camera->ProcessKeyboard(Camera_DOWN, deltaTime);

  if (inputManager->GetScroll() < 0) camera->ProcessKeyboard(Camera_BACKWARD, deltaTime * 4.0f);
  if (inputManager->GetScroll() > 0) camera->ProcessKeyboard(Camera_FORWARD, deltaTime * 4.0f);
  ...
  if (inputManager->GetKeyPress(Keycode::E)) ImGuiLayer::SetGizmoState(ImGuizmo::OPERATION::TRANSLATE);

  if (inputManager->GetKeyPress(Keycode::R)) ImGuiLayer::SetGizmoState(ImGuizmo::OPERATION::ROTATE);

  if (inputManager->GetKeyPress(Keycode::T)) ImGuiLayer::SetGizmoState(ImGuizmo::OPERATION::SCALE);
 }
}
