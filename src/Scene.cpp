...
Scene::Scene(Camera* cam, std::shared_ptr<ResourceHub> resHub) : m_camera(cam)
{
    ...
    testingMesh.GetComponent<MeshComponent>().mesh =
        m_resourceHub->GetMeshManager()->Load(std::make_shared<Mesh>("Testing Mesh"));
    ...
    for (int i = -cubeCount; i < cubeCount; i++) {
        for (int x = -cubeCount; x < cubeCount; x++) {
            CreateEntity("Testing Cube")
                ...
                .SetPosition(glm::vec3((i * 3.0f) + 1.5f, 0.0f, (x * 3.0f)));
        }
    }
...
void* Scene::Render([[maybe_unused]] float deltaTime)
{
    UpdateAndBindRenderTexture();

    // Draw SkyBox
    for (auto view = m_registry.view<SkyBoxComponent>(); auto entityID : view) {
