...
void ContentBrowser::AddTextures(const std::shared_ptr<ResourceHub> &resHub)
{
 for (auto &dirEntry : std::filesystem::directory_iterator(currentDirectory)) {
  const std::string &path = dirEntry.path().string();

  if (!(dirEntry.path().extension() == ".png" || dirEntry.path().extension() == ".jpg")) continue;
...
void ContentBrowser::OnRender(
    const std::shared_ptr<ResourceHub> &resHub, ImVec2 panelPos, ImVec2 panelSize, ImGuiWindowFlags flags)
{
 ...
 if (coloumnCount < 1) coloumnCount = 1;
