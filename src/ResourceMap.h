...
public:
    ...
    T *Add(const std::string &resourcename, const std::string &filename, void *args = 0) {
        ...
        if (it == Map.end()) {
            ...
            if (IsValNonUnique(FileName)) {
                ...
                Log::Error("Filename name %s must be unique\n", FileName.c_str());
            }
        }

        // if we get here means that resource name is duplicated
        Log::Error("Resource name %s must be unique\n", ResourceName.c_str());
        return nullptr;
    }
    ...
    T *Get(const std::string &resourcename) {
        ...
        if (Verbose) {
            Log::Trace("%-64s: %s\n", Name.c_str(),
                       CStringFormatter::Format("Looking for %s", ResourceName.c_str()).c_str());
        }
    ...
    void Initialise(const std::string &name, ResourceManager *resourcemanager, bool verbose, bool duplicates) {
        ...
        if (name.empty())
            Log::Error("Array name cannot be null");
