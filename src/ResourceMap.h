...
public:
 ...
 {
  if (ResourceManager == NULL) Log::Error("DataBase cannot be NULL (5)");

  if (filename.empty()) Log::Error("%s : filename cannot be null", Name.c_str());

  if (resourcename.empty()) Log::Error("%s : resourcename cannot be null", Name.c_str());
 ...
 {
  if (ResourceManager == NULL) Log::Error("DataBase cannot be NULL (4)");

  if (resourcename.empty()) Log::Error("%s : resourcename cannot be null", Name.c_str());

  std::string ResourceName = CStringFormatter::TrimAndLower(resourcename);

  if (Verbose) Log::Trace("%-64s: Removal proposal for : %s\n", Name.c_str(), ResourceName.c_str());
 ...
 Dump()
 {
  if (ResourceManager == NULL) Log::Error("DataBase cannot be NULL (3)");
 ...
 {
  if (ResourceManager == NULL) Log::Error("DataBase cannot be NULL (2)");
 ...
 {
  if (resourcemanager == NULL) Log::Error("DataBase cannot be NULL 1");

  if (name.empty()) Log::Error("Array m_name cannot be null");
 ...
 ~ResourceMap()
 {
  if (Verbose) Log::Trace("%-64s: Releasing\n", Name.c_str());
