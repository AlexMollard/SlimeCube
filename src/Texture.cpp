...
Texture::Texture(std::string name, const std::string& dir) {
    ...
    if (data) {
        glTexImage2D(GL_TEXTURE_2D, 0, nrChannels != 4 ? GL_RGB : GL_RGBA, width, height, 0,
...
Texture::Texture(std::string name, unsigned int id) : textureID(id){
    this->name = std::move(name);
}

Texture::Texture(std::string name, const unsigned int *id)  : textureID(*id){
    this->name = std::move(name);
}
