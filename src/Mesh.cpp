...
void Mesh::Create(Primitives::TYPE type, float argOne, float argTwo, int argThree)
{
 ...
 // bind vertex array aka a m_testingMesh wrapper
...
bool Mesh::Load(const char *filename, bool loadTextures, bool flipTextureV)
{
 ...
 for (auto &m : materials) {
  if (materials.size() > 1) {
   ...
   if (m.ambient_texname.size() > 1) m_materials[index].ambientTexture.Load((folder + m.ambient_texname).c_str());

   if (m.diffuse_texname.size() > 1) m_materials[index].diffuseTexture.Load((folder + m.diffuse_texname).c_str());

   if (m.specular_texname.size() > 1) m_materials[index].specularTexture.Load((folder + m.specular_texname).c_str());
   ...
   if (m.bump_texname.size() > 1) m_materials[index].normalTexture.Load((folder + m.bump_texname).c_str());

   if (m.displacement_texname.size() > 1) m_materials[index].displacementTexture.Load((folder + m.displacement_texname).c_str());
  }
  ++index;
 }
 ...
 for (auto &shape : shapes) {
  ...
  // calculateTangents(vertices, shape.m_testingMesh.indices);
 }
...
void Mesh::Draw(bool usePatches)
{
 if (m_materials.size() > 1) {
  ...
  if (diffuseTexUniform >= 0) glUniform1i(diffuseTexUniform, 0);
  if (specTexUniform >= 0) glUniform1i(specTexUniform, 1);
  if (normalTexUniform >= 0) glUniform1i(normalTexUniform, 2);
  if (ambientTexUniform >= 0) glUniform1i(ambientTexUniform, 3);
  if (specHighlightTexUniform >= 0) glUniform1i(specHighlightTexUniform, 4);
  if (dispTexUniform >= 0) glUniform1i(dispTexUniform, 5);

  // draw the m_testingMesh chunks
  for (auto &c : m_meshChunks) {
   if (m_materials[c.materialID].diffuseTexture.GetID() != 0) BindTexture(&m_materials[c.materialID].diffuseTexture, 0);

   if (m_materials[c.materialID].specularTexture.GetID() != 0) BindTexture(&m_materials[c.materialID].specularTexture, 1);

   if (m_materials[c.materialID].normalTexture.GetID() != 0) BindTexture(&m_materials[c.materialID].normalTexture, 2);

   if (m_materials[c.materialID].ambientTexture.GetID() != 0) BindTexture(&m_materials[c.materialID].ambientTexture, 3);
   ...
   if (m_materials[c.materialID].displacementTexture.GetID() != 0) BindTexture(&m_materials[c.materialID].displacementTexture, 5);
